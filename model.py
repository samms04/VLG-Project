...
def color_constancy_loss(x):
    ...
    ccl=tf.sqrt(tf.square(d_rg)+tf.square(d_rb)+tf.square(d_gb))
...
def exposure_loss(x,mean_val=0.6):
    ...
    el=tf.reduce_mean(tf.square(mean-mean_val))
    return el
...
def illumination_smoothness_loss(x):
    ...
    isl=2*(h_tv/count_h+w_tv/count_w)/batch_size
    return isl
...
class ZeroDCE(keras.Model):
    ...
    def compute_losses(self,data,output):
        ...
        loss_spatial_constancy=tf.reduce_mean(self.spatial_constancy_loss(enhanced_img,data))
        loss_color_constancy=5*tf.reduce_mean(color_constancy_loss(enhanced_img))
        loss_exposure=10*tf.reduce_mean(exposure_loss(enhanced_img))
        tot_loss=(loss_illumination+loss_spatial_constancy+loss_color_constancy+loss_exposure)

        return { "total_loss":tot_loss,
